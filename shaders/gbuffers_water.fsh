 ...
 * 
 * Description: Fragment shader for water, waterfalls, lava, and translucent blocks
 *              Uses triplanar projection for proper dithering on all surface orientations
 */
 ...
 * This ensures waterfalls and vertical surfaces get proper dithering
 */
vec2 getTriplanarUV(vec3 pos, vec3 normal) {
    ...
    vec2 uvX = pos.yz;  // YZ plane (facing X - east/west walls)
    vec2 uvY = pos.xz;  // XZ plane (facing Y - floors/ceilings)
    vec2 uvZ = pos.xy;  // XY plane (facing Z - north/south walls)
    
    // Blend based on normal direction
    // This automatically selects the best UV based on surface orientation
    vec2 uv = uvX * blend.x + uvY * blend.y + uvZ * blend.z;
...
void main() {
    ...
    // Use triplanar projection for surface-stable UV
    // This works for horizontal water AND vertical waterfalls/lava
    ...
    // Preserve alpha
    float alpha = albedo.a * glcolor.a;
    
    /* DRAWBUFFERS:0 */
    gl_FragData[0] = vec4(dithered, alpha);
}
